...
export default function ExpensesPage() {
  ...
  const checkUser = async () => {
    ...
    if (branchesData && branchesData.length > 0) {
      ...
      await fetchExpenses(branchesData[0].id, startDate, endDate)
    } else {
      setIsLoading(false)
    }
  }

  const fetchExpenses = async (branchId: string, start: string, end: string) => {
    setIsLoading(true)
  ...
  const handleFilter = () => {
    if (selectedBranch) fetchExpenses(selectedBranch, startDate, endDate)
  }
  ...
  const uploadFile = async () => {
    ...
    const { data } = supabase.storage.from('finance_attachments').getPublicUrl(filePath)
  ...
  if (isLoading && !profile) return (
    <div className="flex flex-col h-screen items-center justify-center bg-slate-50 text-slate-500">
      ...
      <p className="font-medium animate-pulse">Memuat data pengeluaran...</p>
    </div>
  )
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col pb-20">
      <main className="flex-1 max-w-6xl w-full mx-auto px-4 sm:px-6 py-6 md:py-8">
        ...
        {/* Total Summary */}
        <div className="bg-red-50 border border-red-100 rounded-2xl p-6 mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 relative overflow-hidden">
          <div className="absolute -right-6 -top-6 text-red-500/10 pointer-events-none">
            <Receipt size={120} />
          </div>
          <div className="relative z-10">
            <h2 className="text-sm font-black text-red-800 uppercase tracking-widest">Total Pengeluaran</h2>
            <p className="text-xs font-semibold text-red-600/80 mt-1">Sesuai filter periode dan cabang di atas.</p>
          </div>
          <div className="text-3xl md:text-4xl font-black text-red-700 tracking-tight relative z-10">
            ...
            <table className="min-w-full divide-y divide-slate-200 text-left text-sm">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  ...
                  <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-center">Bukti</th>
              ...
              <tbody className="bg-white divide-y divide-slate-100">
                {isLoading ? (
                  ...
                  <tr key={item.id} className={`${index % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'} hover:bg-red-50/50 transition-colors`}>
                    ...
                    <td className="px-6 py-4 text-slate-600 font-medium max-w-[200px] truncate" title={item.description}>
                      {item.description || '-'}
                    </td>
                    <td className="px-6 py-4 font-black text-red-600 text-right whitespace-nowrap">
                      {formatRp(item.amount)}
                    </td>
                    <td className="px-6 py-4 text-center whitespace-nowrap">
                      {item.attachment_url ? (
                        <a href={item.attachment_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-blue-600 bg-blue-50 hover:bg-blue-100 px-2.5 py-1 rounded-md transition-colors text-xs font-bold border border-blue-100">
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 mb-1.5 uppercase tracking-wider">Bukti Struk/Invoice (Opsional)</label>
                <input
                  type="file"
                  accept="image/jpeg, image/png, application/pdf"
                  onChange={e => setFile(e.target.files?.[0] || null)}
                  className="w-full p-2 border border-slate-200 bg-slate-50 rounded-xl text-sm font-medium text-slate-600 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-xs file:font-bold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 transition-all"
                />
                <p className="text-[11px] font-semibold text-slate-400 mt-2">Format JPG, PNG, atau PDF. Maksimal 5MB.</p>
              </div>
